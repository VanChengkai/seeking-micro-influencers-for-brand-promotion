...
with graph1.as_default():
    ...
    brand_feature=tf.concat([brands_text,normal_brands_pic],axis=1)
    in_feature=tf.concat([influencers_text,normal_influencers_pic],axis=1)
    
    
    
    w_brand_pic1=get_weights([pic_size+text_size,pic_layer1_size],zzh)
    dropout3=tf.nn.dropout(w_brand_pic1,keep_prob)
    b_brand_pic1=tf.Variable(tf.zeros([pic_layer1_size],name="bias_brands_pic_1"))
    w_in_pic1=get_weights([pic_size+text_size,pic_layer1_size],zzh)
    dropout4=tf.nn.dropout(w_in_pic1,keep_prob)
    b_in_pic1=tf.Variable(tf.zeros([pic_layer1_size],name="bias_influencers_pic_1"))


    brand_pic_embed_v1=tf.nn.leaky_relu(tf.matmul(brand_feature,dropout3)+b_brand_pic1,0.01)
    in_pic_embed_v1=tf.nn.leaky_relu(tf.matmul(in_feature, dropout4)+b_in_pic1,0.01)
    w_brand_pic4=get_weights([pic_layer1_size,pic_layer4_size],zzh)
    w_in_pic4=get_weights([pic_layer1_size,pic_layer4_size],zzh)

    b_brand_pic4=tf.Variable(tf.zeros([pic_layer4_size],name="bias_brands_pic_4"))
    b_in_pic4=tf.Variable(tf.zeros([pic_layer4_size],name="bias_in_pic_4"))

    dropout7=tf.nn.dropout(w_brand_pic4,keep_prob)
    dropout8=tf.nn.dropout(w_in_pic4,keep_prob)
    brand_pic_embed_v4=tf.nn.leaky_relu(tf.matmul(brand_pic_embed_v1,dropout7)+b_brand_pic4,0.01)
    in_pic_embed_v4=tf.nn.leaky_relu(tf.matmul(in_pic_embed_v1,dropout8)+b_in_pic4,0.01)
    
    
    
    
    product_1=tf.multiply(brand_pic_embed_v4,in_pic_embed_v4)
    ...
    with tf.Session(graph=graph1) as sess:
        ...
        for j in range(0,Epoch_):
            ...
            for i in range(0,part_):
                ...
                #print('part:',i)
                print('loading...train')
                path1='F:\\sampling\\dataset_k3\\brand_text_train_'+str(i)+'.npy'
                brand_text_train=np.load(path1)
                print('loading...train')
                in_text_train=np.load('F:\\sampling\\dataset_k3\\in_text_train_'+str(i)+'.npy')
                print('loading...train')
                brand_pic_train=np.load('F:\\sampling\\dataset_k3\\brand_pic_train_'+str(i)+'.npy')
                print('loading...train')
                in_pic_train=np.load('F:\\sampling\\dataset_k3\\in_pic_train_'+str(i)+'.npy')
                print('loading...train')
                labels_train=np.load('F:\\sampling\\newlabel\\label_train_'+str(i)+'.npy')
                ...
                for step in xrange(Step_):
                    ...
                    _brand_pic,_brand_embed, _in_embed_v4, _labels, _x,_y=sess.run([y_2,brand_pic_embed_v4, x, labels,w_brand_pic1, w_brand_pic4],feed_dict=feed_dict)
                    ...
                    globalstep=part+(19*Epoch)+1+266
                    if (  (globalstep%19==0 or (globalstep%19==10 ) )  and (step==Step_-1) and globalstep!=0):
