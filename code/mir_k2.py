...
def get_batch(brand_text_,brand_pic_,in_text_,in_pic_,labels_,step):
    if((step+1)*batch_size*2>len(brand_text_)):
        brand_text=brand_text_[step*batch_size*2:]
        brand_pic=brand_pic_[step*batch_size*2:]
        in_text=in_text_[step*batch_size*2:]
        in_pic=in_pic_[step*batch_size*2:]
        label_=labels_[step*batch_size*2:]
        label_ = label_.reshape([batch_size*2])
    else:
        brand_text=brand_text_[step*batch_size*2:(step+1)*batch_size*2]
        brand_pic=brand_pic_[step*batch_size*2:(step+1)*batch_size*2]
        in_text=in_text_[step*batch_size*2:(step+1)*batch_size*2]
        in_pic=in_pic_[step*batch_size*2:(step+1)*batch_size*2]
        label_=labels_[step*batch_size*2:(step+1)*batch_size*2]
        label_ = label_.reshape([batch_size*2])
        #print('label:',label_)
...
with graph1.as_default():
    ...
    brands_pic_=tf.reshape(brands_pic,[batch_size*2,7,7,512])
    influencers_pic_=tf.reshape(influencers_pic,[batch_size*2,7,7,512])
    ...
    normal_brands_pic=tf.reshape(normal_brands_pic,[batch_size*2,25088])
    normal_influencers_pic=tf.reshape(normal_influencers_pic,[batch_size*2,25088])
    
    
    w_brand_text1=w_brand_text1=get_weights([text_size,text_layer1_size],zzh)
    ...
    y=tf.reshape(x,[batch_size,2])
    y_1=tf.nn.softmax(y)
    y_2=tf.reshape(y_1,[batch_size*2])
    y_2=y_2+1e-8
    ...
    with tf.Session(graph=graph1) as sess:
        ...
        for j in range(0,Epoch_):
            ...
            for i in range(0,part_):
                ...
                print('loading...train')
                path1='G:\\aaa\\dataset_k2\\brand_text_train_'+str(i)+'.npy'
                brand_text_train=np.load(path1)
                print('loading...train')
                in_text_train=np.load('G:\\aaa\\dataset_k2\\in_text_train_'+str(i)+'.npy')
                print('loading...train')
                brand_pic_train=np.load('G:\\aaa\\dataset_k2\\brand_pic_train_'+str(i)+'.npy')
                print('loading...train')
                in_pic_train=np.load('G:\\aaa\\dataset_k2\\in_pic_train_'+str(i)+'.npy')
                print('loading...train')
                labels_train=np.load('G:\\aaa\\dataset_k2\\label_train_'+str(i)+'.npy')
                ...
                for step in xrange(Step_):
                    ...
                    _brand_pic,_brand_embed, _in_embed_v4, _labels, _x,_y=sess.run([y_2,brand_text_embed_v4, x, labels,w_brand_pic4, w_in_pic4],feed_dict=feed_dict)
                    ...
                    globalstep=part+(10*Epoch)+1
                    if (globalstep%10==0 and (step==Step_-1) and globalstep!=0):
                ...
                if ((part+1)%10==0):
                    ...
                    for k in range(0,2):
                        print('loading...test')
                        path1='F:\\dataset_k_2_test\\brand_text_test_'+str(k)+'.npy'
                        brand_text_test=np.load(path1)
                        print('loading...test')
                        in_text_test=np.load('F:\\dataset_k_2_test\\in_text_test_'+str(k)+'.npy')
                        print('loading...test')
                        brand_pic_test=np.load('F:\\dataset_k_2_test\\brand_pic_test_'+str(k)+'.npy')
                        print('loading...test')
                        in_pic_test=np.load('F:\\dataset_k_2_test\\in_pic_test_'+str(k)+'.npy')
                        print('loading...test')
                        labels_test=np.load('F:\\dataset_k_2_test\\label_test_'+str(k)+'.npy')
